...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--left-bound', type=int, metavar='N', default=1,
                            help='the left search interval for the first token')
        parser.add_argument('--right-bound', type=int, metavar='N', default=5,
                            help='the right search interval for the first token')
        parser.add_argument('--action-loss-smoothing', default=0., type=float, metavar='D',
                            help='epsilon for label smoothing, 0 means no label smoothing')
        parser.add_argument('--multipath-training', default=False, action='store_true',
                            help='if set, use multipath training')
        parser.add_argument('--classerifier-training', default=False, action='store_true',
                            help='if set, use classerifier training')
        # fmt: on
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens,encoder_embde_tokens=None, no_encoder_attn=False):
        ...
        self.left_bound = args.left_bound
        self.right_bound = args.right_bound
        self.action_loss_smoothing = args.action_loss_smoothing
        self.multipath_training = args.multipath_training
        self.classifier_training = args.classerifier_training
        
    def forward(self, prev_output_tokens, encoder_out=None, emd_fc=None, act_fc=None, out=None,lstm=None, action_embedding=None, incremental_state=None, new_times=5, reorder=None,step=None, **unused):
        """
        Args:
        ...
        """
        x, extra = self.extract_features(prev_output_tokens, encoder_out, incremental_state, new_times=new_times, reorder=reorder,step=step)

        if self.training == False and step is not None:
            dec_hidd = extra['decoder_hidSta']
            enc_hidd = reorder
            res = self.action_generation(step, lstm, action_embedding, emd_fc, act_fc, out, enc_hidd, dec_hidd)
            extra['direction'] = torch.softmax(res, dim=-1)

        x = self.output_layer(x)
        return x, extra
    
    # judge the next action
    def action_generation(self, division, lstm, embedd, emd_fc, act_fc, out, enc_hidd, dec_hidd):
        bsz = division.size(0)
        tgt_len = dec_hidd.size(1)
        hidd_dim = dec_hidd.size(-1)

        max_src = torch.max(division[:, -1], dim=-1, keepdim=False)[0]
        tempor = torch.arange(division.size(1)).cuda().unsqueeze(0)
        action_seq = torch.zeros(division.size(0), division.size(1)+max_src).cuda()
        action_seq = action_seq.scatter(-1, (division + tempor).long(), 1)
        action_embedding = embedd(action_seq.int())
        ...
        dec_hidd = torch.gather(dec_hidd, 1, tgt_index.long().unsqueeze(-1).expand(bsz, tgt_len+max_src, hidd_dim))
        dec_hidd = dec_hidd[:, 1:, :]
        src_index = (torch.cumsum(1 - action_seq, dim=-1) - (1 - action_seq) - 1)[:, 1:]
        enc_hidd = torch.gather(enc_hidd, 1, src_index.long().unsqueeze(-1).expand(bsz, tgt_len+max_src-1, hidd_dim))
        
        midd_fea = emd_fc(torch.cat((enc_hidd, dec_hidd), dim=-1))
        midd_fea = F.relu(midd_fea)
        action_embedding = F.relu(act_fc(action_embedding))
        midd_fea = torch.cat((midd_fea, action_embedding), dim=-1)
        midd_fea, (_, _) = lstm(midd_fea.transpose(0, 1))
        midd_fea = midd_fea.transpose(0, 1)
        midd_fea = torch.gather(midd_fea, 1, (division[:, -1].unsqueeze(-1) + tgt_len).long().unsqueeze(-1).expand(bsz, 1, 512)-2)
        midd_fea = midd_fea.squeeze(1)
        midd_fea = out(midd_fea)

        return midd_fea
    def extract_features(self, prev_output_tokens, encoder_out=None, incremental_state=None, new_times=5, reorder=None, step=None, **unused):
        """
        ...
        for layer in self.layers:
            ...
            if attn is not None and calNumber==6:
                attn_weights = attn
#        if attn is not None:
#            attn_wei_size = attn_weights[0].size()
#            attn_weights_tmp = torch.cat(attn_weights, 0)
#            attn_weights = attn_weights_tmp.sum(dim=0).reshape(attn_wei_size[-3], attn_wei_size[-2], attn_wei_size[-1]) / attn_weights_tmp.size(0)
        if attn is None:
            attn_weights = None
        ...
        return x, {'attn': attn_weights, 'inner_states': inner_states, 'decoder_hidSta': tmp1}
